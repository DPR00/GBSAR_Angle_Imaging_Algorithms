...
def get_SAR_data():
    ...
    It, Rt = sp.get_scalar_data3() # Coordenadas(m) y magnitud del target respectivamente
...
def RMA_Algorithm(data1):
    ...
    kx=np.fft.fftfreq(len(S3))*2*np.pi/dp
    ...
    for i in range(len(kx)):
        interp_fn1 = sc.interpolate.interp1d(ky[i], S4.real[i], bounds_error=False, fill_value=0)#, kind='cubic')
        interp_fn2 = sc.interpolate.interp1d(ky[i], S4.imag[i], bounds_error=False, fill_value=0)#, kind='cubic')
    ...
    Nkx = len(S6)
    kx_n = np.fft.fftshift(np.fft.fftfreq(ind1)*ind1*(kx[1]-kx[0]))
    ...
    Nky = len(S7[0])
    ky_n = np.arange(ind2)*(ky_it[1]-ky_it[0])+ky_it.min()
    ...
    S8 = np.fft.ifftshift(S7, axes=(0)) # De-centra el eje kx
    ...
    I = S8.T/(np.sqrt(np.sum(abs(S8)**2)))
